...
CREATE TABLE IF NOT EXISTS `ChemDB`.`Published` (
  ...
  `pmid` INT NOT NULL,
  `aeid` INT NOT NULL,
  PRIMARY KEY (`aeid`, `pmid`, `citation_id`),
  CONSTRAINT `fk_Published_Citation1`
    FOREIGN KEY (`citation_id` , `pmid`)
...
SHOW WARNINGS;
CREATE INDEX `fk_Published_Citation1_idx` ON `ChemDB`.`Published` (`citation_id` ASC, `pmid` ASC);

SHOW WARNINGS;
CREATE INDEX `fk_Published_Assay1_idx` ON `ChemDB`.`Published` (`aeid` ASC);
...
SHOW WARNINGS;
CREATE INDEX `fk_Tested_Target1_idx` ON `ChemDB`.`Tested` (`target_id` ASC);

SHOW WARNINGS;
CREATE INDEX `fk_Tested_Assay1_idx` ON `ChemDB`.`Tested` (`aeid` ASC);
