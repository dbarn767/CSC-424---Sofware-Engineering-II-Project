...
if ($_POST['submit']){
	...
	if ($type == "che"){
		...
		if ($weight != NULL){
			...
			$query = $query . $combiner . $original . "WHERE Structure_MolWt LIKE '%".$weight."%'";
			$partCounter += 1;
		}
	...
	elseif ($type == "tox"){
		// NOTE: Only ID can be specified, but it applies to two different primary keys. We need to search both for results and mash those results into one array
		$query = "SELECT * FROM Toxicity";
		if ($id != NULL){
			$query = $query . " WHERE aeid LIKE '%".$id."%' OR Substance_CASRN LIKE '%".$id."%'";
		}
	}
	elseif ($type == "tar"){
	...
	elseif ($type == "asy"){
		...
		if ($id != NULL){
			$query = $query . " WHERE aeid LIKE '%".$id."%'";
		}
	}
	else{
